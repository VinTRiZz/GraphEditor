...
void removeExtraLogs() {
    ...
    logfiles.removeOne("."); logfiles.removeOne(".."); // Игнорируем текущую и директорию выше (кьют багованный в этом плане)
    ...
    if (logfiles.size() > maxLogCount) {
        ...
        auto currentLogfile = QFileInfo(Logging::LoggingMaster::getCurrentLogfile()).baseName();
        std::sort(logfiles.begin(), logfiles.end(),
                  [](const QString& logFilePathLeft, const QString& logFilePathRight) {
            // TODO: Вынести в отдельную функцию из логгера
            // Текущий формат: 20-59-44_03-07-2025
            auto leftBasenameDate = QDateTime::fromString(QFileInfo(logFilePathLeft).baseName(), "");
            auto rightBasenameDate = QDateTime::fromString(QFileInfo(logFilePathRight).baseName(), "");
            return (leftBasenameDate < rightBasenameDate);
        });
        std::reverse(logfiles.begin(), logfiles.end());
        logfiles.erase(logfiles.begin(), logfiles.begin() + maxLogCount); // Игнорируем первые N файлов
