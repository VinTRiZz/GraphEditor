  ...
  if (logfiles.size() > maxLogCount) {
    ...
    std::sort(
        ...
        [](const QString &logFilePathLeft, const QString &logFilePathRight) {
          ...
          auto leftBasenameDate =
              QDateTime::fromString(QFileInfo(logFilePathLeft).baseName(), "");
          auto rightBasenameDate =
              QDateTime::fromString(QFileInfo(logFilePathRight).baseName(), "");
          return (leftBasenameDate < rightBasenameDate);
        });
    std::reverse(logfiles.begin(), logfiles.end());
    logfiles.erase(logfiles.begin(),
                   logfiles.begin() +
                       maxLogCount); // Игнорируем первые N файлов
