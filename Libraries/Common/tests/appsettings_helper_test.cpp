...
TEST(GeneralConfigurationTest, ThemeConversion) {
    ...
    EXPECT_EQ(config.themeToString(GeneralConfiguration::Theme::System),
              "System");
    EXPECT_EQ(config.themeToString(GeneralConfiguration::Theme::Light),
              "Light");
    EXPECT_EQ(config.themeToString(GeneralConfiguration::Theme::Dark), "Dark");

    // Проверка обратного преобразования
    EXPECT_EQ(config.stringToTheme("System"),
              GeneralConfiguration::Theme::System);
    EXPECT_EQ(config.stringToTheme("Light"),
              GeneralConfiguration::Theme::Light);
    EXPECT_EQ(config.stringToTheme("Dark"), GeneralConfiguration::Theme::Dark);

    // Проверка неверных значений
    EXPECT_EQ(config.stringToTheme("Invalid"),
              GeneralConfiguration::Theme::System);
...
TEST(GeneralConfigurationTest, Serialization) {
    ...
    EXPECT_EQ(loaded.getNeedBackwardCompatible(),
              original.getNeedBackwardCompatible());
    ...
    EXPECT_EQ(loaded.getNeedCleanupTempFiles(),
              original.getNeedCleanupTempFiles());
...
TEST(ObjectsConfigurationTest, GettersSetters) {
    ...
    // Проверка значений
    EXPECT_EQ(config.getDefaultNodeShape(),
              ObjectsConfiguration::NodeShape::Hexagon);
...
TEST(ObjectsConfigurationTest, Serialization) {
    ...
    EXPECT_COLOR_EQ(loaded.getNodeSelectionColor(),
                    original.getNodeSelectionColor());

    EXPECT_COLOR_EQ(loaded.getLabelTextColor(), original.getLabelTextColor());
    EXPECT_COLOR_EQ(loaded.getLabelBackgroundColor(),
                    original.getLabelBackgroundColor());
    ...
    EXPECT_COLOR_EQ(loaded.getLineSelectionColor(),
                    original.getLineSelectionColor());
}

int main(int argc, char** argv) {
    QCoreApplication app(
        argc,
        argv);  // Инициализация для работы с QColor и QSettings
