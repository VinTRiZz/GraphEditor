...
void GraphCommonPropertiesModel::setGraph(Graph::PMaintainer pMaintainer) {
  ...
  if (m_pMaintainer) {
    connect(m_pMaintainer.get(), &Graph::GraphMaintainer::changedCommonProperty,
            this, [this]() {
              beginResetModel();
              endResetModel();
            });
...
QVariant GraphCommonPropertiesModel::data(const QModelIndex &index,
  ...
  if (index.column() == 0) {
    ...
    switch (index.row()) {
    case CPR_NAME:
      return m_pMaintainer->getName();
    case CPR_DESCRIPTION:
      return m_pMaintainer->getDescription();
    case CPR_CREATED_BY:
      return m_pMaintainer->getCreateTime().toString(
          GraphCommon::DATE_DISPLAY_CONVERSION_FORMAT);
    case CPR_EDITED_BY:
      return m_pMaintainer->getEditTime().toString(
          GraphCommon::DATE_DISPLAY_CONVERSION_FORMAT);
    }
  }
  return QVariant();
...
Qt::ItemFlags
GraphCommonPropertiesModel::flags(const QModelIndex &index) const {
