...
void GraphCustomPropertiesModel::setGraph(Graph::PMaintainer pMaintainer)
{
    ...
    if (m_pMaintainer) {
        connect(m_pMaintainer.get(), &Graph::GraphMaintainer::changedCustomProperty, this, [this](){
            beginResetModel();
            endResetModel();
        });
    }
...
{
    // TODO: Сделать сложную логику по поиску точки вставки (нет большой необходимости)
...
bool GraphCustomPropertiesModel::setData(const QModelIndex &index, const QVariant &value, int role)
{
    ...
    if (index.column() == 0) {
        ...
        auto customValueKey = index.siblingAtColumn(0).data(Qt::DisplayRole).toString();
...
Qt::ItemFlags GraphCustomPropertiesModel::flags(const QModelIndex &index) const
{
