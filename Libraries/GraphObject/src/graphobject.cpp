...
std::optional<GVertex>
GraphObject::getVertex(GraphCommon::graphId_t vertexId) const {
  auto targetVertex =
      std::find_if(m_vertices.begin(), m_vertices.end(),
...
std::list<GVertex> GraphObject::getAllVertices() const { return m_vertices; }

std::size_t GraphObject::getVerticesCount() const { return m_vertices.size(); }
...
bool GraphObject::addConnection(const GConnection &iCon) {
  ...
  if (std::any_of(
          existingConnection.first, existingConnection.second,
          [&iCon](const auto &con) { return con.second.idTo == iCon.idTo; })) {
    return false;
  }
...
std::vector<GConnection>
GraphObject::getConnectionsFromVertex(GraphCommon::graphId_t vertexId) const {
...
std::optional<GConnection>
GraphObject::getConnection(GraphCommon::graphId_t vertexFromId,
...
void GraphObject::clearConnections() { m_connections.clear(); }
