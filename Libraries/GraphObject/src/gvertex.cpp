...
bool Graph::GVertex::operator==(const GVertex& oVert_) const {
    auto borderColorCompare = (CommonFunctions::encodeColor(borderColor) ==
                               CommonFunctions::encodeColor(oVert_.borderColor));
    auto bgrColorCompare = (CommonFunctions::encodeColor(backgroundColor) ==
...
//        LOG_DEBUG_SYNC(
//            "Colors BORDER:      ", CommonFunctions::encodeColor(borderColor),
//            CommonFunctions::encodeColor(oVert_.borderColor));
//        LOG_DEBUG_SYNC(
//            "Colors BACKGROUND:  ", CommonFunctions::encodeColor(backgroundColor),
//            CommonFunctions::encodeColor(oVert_.backgroundColor));
...
bool Graph::GVertex::operator!=(const GVertex& oVert_) const {
    ...
    auto res = applyOperator(oVert_, std::not_equal_to<>{}) ||
               (std::fabs(posX - oVert_.posX) >
                std::numeric_limits<double>::epsilon()) ||
               (std::fabs(posY - oVert_.posY) >
                std::numeric_limits<double>::epsilon()) ||
