...
bool Graph::GVertex::operator==(const GVertex& oVert_) const {
    auto borderColorCompare = (GraphCommon::encodeColor(borderColor) ==
                               GraphCommon::encodeColor(oVert_.borderColor));
    auto bgrColorCompare = (GraphCommon::encodeColor(backgroundColor) ==
                            GraphCommon::encodeColor(oVert_.backgroundColor));
    ...
    if (!res) {
        ...
        LOG_DEBUG_SYNC(
            "Colors BORDER:      ", GraphCommon::encodeColor(borderColor),
            GraphCommon::encodeColor(oVert_.borderColor));
        LOG_DEBUG_SYNC(
            "Colors BACKGROUND:  ", GraphCommon::encodeColor(backgroundColor),
            GraphCommon::encodeColor(oVert_.backgroundColor));
