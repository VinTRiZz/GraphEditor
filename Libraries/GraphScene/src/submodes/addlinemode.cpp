...
void AddLineMode::clearMode()
{
    if (nullptr != m_pendingConnectionLine) {
      getParentMode()->getScene()->removeObject(m_pendingConnectionLine->getObjectId());
    }
...
void AddLineMode::processMove(QGraphicsItem *pTargetItem, const QPointF &currentPos)
{
    if (nullptr != m_pendingConnectionLine) {
...
{
    auto pTargetVertexItem = dynamic_cast<ObjectViewItems::ItemBase*>(pTargetItem);
