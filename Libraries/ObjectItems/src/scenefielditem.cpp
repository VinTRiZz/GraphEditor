...
ItemBase *SceneFieldItem::getItem(ObjectViewConstants::objectId_t targetItemId, ObjectViewConstants::ObjectType targetItemType) const
{
...
std::list<ObjectViewConstants::objectId_t> SceneFieldItem::getRegisteredIds() const
{
    std::list<ObjectViewConstants::objectId_t> res;
    for (auto& [itemType, items] : m_registeredItems) {
        std::transform(items.begin(), items.end(), std::back_inserter(res), [](auto* pItem){
...
void SceneFieldItem::removeRegisteredItem(ItemBase* pItem) {
    ...
    for (auto pRegItem : targetItems) {
        if (pRegItem == pItem) {
            delete pRegItem;
            targetItems.erase(std::find(targetItems.begin(), targetItems.end(), pItem));
            break;
        }
    }
}

void SceneFieldItem::removeRegisteredItemById(ObjectViewConstants::objectId_t targetItemId)
{
    for (auto& [itemType, items] : m_registeredItems) {
        for (auto pItem : items) {
            if (pItem->getObjectId() == targetItemId) {
