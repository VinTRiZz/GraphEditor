...
TEST(SceneFieldItemTest, SetFieldRect) {
  auto fieldItem = std::shared_ptr<ObjectViewItems::SceneFieldItem>(
      new ObjectViewItems::SceneFieldItem);
...
TEST(SceneFieldItemTest, RegisterAndRemoveItems) {
  ...
  EXPECT_EQ(fieldItem->getItem(
                2, ObjectViewConstants::ObjectType::OBJECTTYPE_ARROWLINE),
            item2);
  EXPECT_EQ(fieldItem->getItem(
                3, ObjectViewConstants::ObjectType::OBJECTTYPE_PROPERTY_EDITOR),
            item3);
...
TEST(SceneFieldItemTest, ItemRemoval) {
  ...
  fieldItem->removeRegisteredItems(
      ObjectViewConstants::ObjectType::OBJECTTYPE_ARROWLINE);
