...
InteractiveObjectView::InteractiveObjectView(QWidget *parent)
  ...
  p.drawEllipse(QRectF(maxX / 2.0 - centerSizeX / 2.0,
                       maxY / 2.0 - centerSizeY / 2.0, centerSizeX,
                       centerSizeY));

  p.setPen(QPen(QColor(255, 0, 0, 170), 3));
  p.setBrush(Qt::transparent);
  p.drawEllipse(QRectF(maxX * (1 - roundEllipseScale) * 0.5,
...
void InteractiveObjectView::setCanvasRect(const QRectF &iRect) {
  ...
  m_pCenterItem->setPos(iRect.center() -
                        m_pCenterItem->boundingRect().center());
  updateCenterPoint();
}
...
void InteractiveObjectView::setGrabObject(QGraphicsItem *pItem) {
  ...
  m_grabObjectId = pItem->data(ObjectViewConstants::ObjectField::OBJECTFIELD_ID)
                       .toLongLong();
...
void InteractiveObjectView::updateCenterPoint() {
  auto targetPos = getCanvasRect().center() -
                   (m_pCenterItem->boundingRect().center() / getCurrentScale());
...
void InteractiveObjectView::updateCenterMarker() {
  ...
  QPointF targetPos =
      m_pCenterItem->scenePos() + m_pCenterItem->boundingRect().center();
  ...
  if (viewportRect.contains(targetPos)) {
    ...
    m_centerPointerItem->setPos(
        targetLine.pointAt(viewportRect.height() * 0.4 / targetLine.length()));
  }
}
